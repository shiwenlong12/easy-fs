...
fn easy_fs_pack() -> std::io::Result<()> {
    //使用 clap crate 进行命令行参数解析，需要通过 -s 和 -t 分别指定应用的源代码目录和保存应用 ELF 的目录，
    //而不是在 easy-fs-fuse 中硬编码。
    ...
    //并将数据读入内存。接着需要在 easy-fs 中创建一个同名文件并将 ELF 数据写入到这个文件中。
    //这个过程相当于将 Linux 上的文件系统中的一个文件复制到我们的 easy-fs 中。
    for app in apps {
